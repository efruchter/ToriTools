...
public class SampleGame {
	...
	public void init() throws IOException {
		...
		frame.setCursor(hiddenCursor); // Cursor hidden by default
	...
	private void logic() {
		...
		debug = keys.isPressedThenReleased(KeyEvent.VK_K) ? !debug : debug; // debug
																			// control
		/*
		 * Escape key.  Potentially to be used for menu access.
		 */
		if (keys.isPressed(KeyEvent.VK_ESCAPE))
		{
			setFullScreen(false);
			frame.setCursor(null); // Restores cursor for menu use
			System.exit(0);
		}
	}
	...
	/**
	 * Display mode of user's monitor.  Uses 32-bit color.
	 */
