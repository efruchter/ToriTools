...
public class Universe {
	...
	public Body addEntity(final Entity ent, final BodyType bodyType,
			final boolean allowRotation, final boolean spherical,
			final float density, final float friction, final Vector2[] points,
		...
		Shape p = null;
		if (points == null) {
			bd.position.set(ent.getPos().x / PTM_RATIO + ent.getDim().x / 2
					/ PTM_RATIO, ent.getPos().y / PTM_RATIO + ent.getDim().y
					/ 2 / PTM_RATIO);
			if (!spherical) {
				p = new PolygonShape();
				((PolygonShape) p).setAsBox(ent.getDim().x / PTM_RATIO / 2,
						ent.getDim().y / PTM_RATIO / 2);
			} else {
				p = new CircleShape();
				p.m_radius = ent.getDim().x / PTM_RATIO / 2;
			}
		} else {
			bd.position.set(V2(ent.getPos().add(ent.getDim().scale(.5f))
					.scale(1f / PTM_RATIO)));
			PolygonShape ss = new PolygonShape();
			ss.setAsEdge(V2(points[0].scale(1f / PTM_RATIO)),
					V2(points[1].scale(1f / PTM_RATIO)));
			p = ss;
		}
	...
	public Body addEntity(final Entity ent, final BodyType bodyType,
			final boolean allowRotation, final boolean spherical,
			final float density, final float friction) {
		return addEntity(ent, bodyType, allowRotation, spherical, density,
				friction, null, false);
	}
	...
	public Joint addHinge(Entity a, Entity b, final Vector2 hingePosition) {
		...
		def.initialize(bodyMap.get(a), bodyMap.get(b),
				V2(hingePosition.scale(1f / PTM_RATIO)));
		return world.createJoint(def);
	}

	public Joint addWeld(Entity a, Entity b) {
		WeldJointDef def = new WeldJointDef();
		def.initialize(bodyMap.get(a), bodyMap.get(b), bodyMap.get(a)
				.getWorldCenter());
		return world.createJoint(def);
	}
	...
	private final EntityScript serviceScript = new EntityScriptAdapter() {
		...
		public void onUpdate(Entity self, long time, Level level) {
			...
			Vector2 newPos = new Vector2((body.position.x * PTM_RATIO)
					- self.getDim().x / 2, body.position.y * PTM_RATIO
					- self.getDim().y / 2);
	...
	public void applyForce(final Entity e, final Vector2 force) {
		...
		if (b != null)
			b.applyForce(V2(force.scale(1f / b.m_mass)), b.getWorldCenter());
	}

	public void applyLinearImpulse(final Entity e, final Vector2 force) {
		Body b = bodyMap.get(e);
		if (b != null)
			b.applyLinearImpulse(V2(force.scale(1f / b.m_mass)),
					b.getWorldCenter());
	}
	...
	public void setTransform(final Entity e, final Vector2 pos,
			final float angleDeg) {
		bodyMap.get(e).setTransform(V2(pos.scale(1f / PTM_RATIO)),
				angleDeg / 57.3f);
	}

	private Vec2 V2(final Vector2 v) {
